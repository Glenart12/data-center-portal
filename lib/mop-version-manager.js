...
/**
 * Get the next version number for a MOP based on equipment number and date
 ...
 */
export function getNextVersion(existingFiles, equipmentNumber, currentDate = null) {
  ...
  // Check all existing files for matching equipment number and date
  existingFiles.forEach(file => {
    // Handle both string filenames and blob objects
    const filename = typeof file === 'string' ? file : (file.pathname || file.url || '');
    
    // Check if filename contains the equipment number and same date
    ...
    // If both equipment number and date match, extract version
    if (hasEquipmentNumber && hasDate) {
